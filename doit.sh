...
# Final cleanup - remove ClickHouse
echo "🧹 Removing ClickHouse (no longer needed)..."
...
# Remove volume
docker volume rm n8n-ai_clickhouse_data 2>/dev/null

# Remove config directory
rm -rf clickhouse_config

# Update docker-compose.yml to remove ClickHouse
cp docker-compose.yml docker-compose.yml.temp

# Create final clean docker-compose.yml without version and without ClickHouse
...
echo "✅ Clean docker-compose.yml created (no version, no ClickHouse)"
...
echo ""
echo "📁 Place your code in: ./code_repos/"
echo "📊 Check logs: docker compose logs -f [service]"
echo ""
echo "🎉 Your AI Code Assistant is ready!"
